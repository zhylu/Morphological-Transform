...
traindata,testdata = torch.from_numpy(numpy.array(data['cifar100train'],dtype=numpy.float32)).permute(0,1,3,2),\
                     torch.from_numpy(numpy.array(data['cifar100test'],dtype=numpy.float32)).permute(0,1,3,2) 
                     
trainlabels,testlabels = numpy.array(data['cifar100trainfinelabels']),numpy.array(data['cifar100testfinelabels']) 
labels = torch.tensor(trainlabels).squeeze()

print(traindata.shape,testdata.shape,trainlabels.shape,testlabels.shape,labels.shape)
#%%
sd = 32
...
tfm = tf.Compose([tf.ToPILImage(),
                  tf.ToTensor(),                    
                  tf.Normalize([0.5074, 0.4867, 0.4411],[0.2675,0.2566,0.2763])])
...
for epoch in range(200):
    ...
    if epoch > 80 :
        ...
        traindatafe = numpy.zeros((50000,d))
        ...
        with torch.no_grad():
            ...
            classifier= LogisticRegression(solver='saga', multi_class='multinomial', tol=.01, C=100., max_iter=100)    
            scaler = MinMaxScaler()
