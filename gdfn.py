...
import torch
import numpy
traindatan = numpy.array(data['cifar10train'])
traindata = torch.from_numpy(traindatan).permute(0,1,3,2)
...
m = gdfn_region_batch()
test = m(traindata[0:100].cuda())
# test = test.unsqueeze(0)
#%%
import matplotlib.pyplot as plt
n = 0
t= traindata[n].permute(1,2,0).cpu().float()/255

plt.imshow(t)
plt.show()

t1= test[n].permute(1,2,0).cpu().float()/255

plt.imshow(t1)
plt.show()

t2= (test[n].cpu()-traindata[n].cpu()).permute(1,2,0).float()/255
